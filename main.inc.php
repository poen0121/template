		...
		public static function view($path = null, $data = array (), $process = false) {
			if (!hpl_func_arg :: delimit2error() && !hpl_func_arg :: string2error(0) && !hpl_func_arg :: array2error(1) && !hpl_func_arg :: bool2error(2)) {
				if (isset ($path { 0 })) {
					...
					if (!hpl_path :: is_absolute($normPath) && is_file($normPath) && is_readable($normPath) && preg_match('/^(.)*\.php$/i', $normPath)) {
						if (!ob_start()) {
						...
						hpl_error :: cast(__CLASS__ . '::' . __FUNCTION__ . '(): Unable to load template file ' . $normPath, E_USER_NOTICE, 1);
					}
				} else {
